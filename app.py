...
def user_percentile(total_emissions):
    ...
    middle_income = np.random.normal(loc=2.1, scale=0.5, size=4000)     # 40%
...
with tabs[3]:
    ...
    if total_emissions < 6.7:
        ...
        st.markdown("<div class='subtitle'>Your annual footprint is **below the global average**. Keep it up</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div class='main-title'>🚨 Heads up!</div>", unsafe_allow_html=True)
        st.markdown("<div class='subtitle'>Your annual footprint is **above the global average**. Check how much of a differnence can your bring with small changes in your lifestyle.</div>", unsafe_allow_html=True)
